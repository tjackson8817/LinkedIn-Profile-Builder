...
7. **Three profile banner images** *(optional toggle, off by default)* — three distinct `.png` banners, programmatically generated with Python's Pillow library via Claude's code execution capability (the same mechanism the LinkedIn Article Share Builder tool uses for its artwork) — not Claude's separate image-generation feature, and not a photorealistic or stock image. Requires the Code execution and file creation setting, and only works when the prompt is run directly in Claude, not other AI tools.
